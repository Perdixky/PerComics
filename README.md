...
**章节文件夹命名规则**：
- ✅ 建议使用数字前缀：`第001话`、`Chapter 01`、`001`
- ✅ 系统会自动进行自然排序（1, 2, 10 而不是 1, 10, 2）
- ✅ 章节列表会**倒序显示**（最新章节在最前）
- 示例命名方案：
  - `001`, `002`, `003` (简洁)
  - `第001话`, `第002话`, `第003话` (中文)
  - `Chapter 01`, `Chapter 02` (英文)
  - `单行本24卷`, `单行本25卷` (卷)
  - `番外01`, `番外02` (番外)
...
#### Q: 章节顺序不对？
**A**:
- 章节会按照**自然排序**后**倒序**显示（最新在前）
- 确保章节文件夹名使用数字前缀：`001`, `002`, `010` 而不是 `1`, `2`, `10`
- 如果要显示为 `1, 2, 10` 而不是 `01, 02, 10`，可以不加前导零，但要注意 `10` 会排在 `2` 前面
